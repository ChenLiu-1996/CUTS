...
def run(test_set, save_path, mode) -> None:
    ...
    for i, (x, y) in enumerate(tqdm(test_set)):
        ...
        if mode == 'generate':
            ...
            label_sam_med2d = np.zeros_like(label)
            for mask_idx in range(len(preds)):
                label_sam_med2d[preds[mask_idx]['segmentation']] = mask_idx+1
        else:
            ...
            if np.max(label) != 1:
                ...
                label_sam_med2d = preds.squeeze(0)

        label_sam_med2d = skimage.transform.resize(label_sam_med2d, (H, W), order=0, preserve_range=True, anti_aliasing=False).astype(np.uint8)
        ...
        with open('%s/%s' % (save_path, 'sample_%s.npz' % str(i).zfill(5)),
                  'wb+') as f:
            np.savez(f, image=x, label=y, label_sam_med2d=label_sam_med2d)
...
if __name__ == "__main__":
    ...
    save_path = '%s/%s' % (config.output_save_path, 'numpy_files_seg_SAM_Med2D')
