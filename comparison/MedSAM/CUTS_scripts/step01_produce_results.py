    ...
    for i, (x, y) in enumerate(tqdm(test_set)):
        ...
        if mode == 'generate':
            ...
            for mask_idx in range(len(preds)):
                label_medsam[preds[mask_idx]['segmentation']] = mask_idx+1
        else:
        ...
        with open('%s/%s' % (save_path, 'sample_%s.npz' % str(i).zfill(5)),
                  'wb+') as f:
            np.savez(f, image=x, label=y, label_medsam=label_medsam)
...
if __name__ == "__main__":
    ...
    save_path = '%s/%s' % (config.output_save_path, 'numpy_files_seg_MedSAM')
